...
void FindItem(const Item * pi, const Tree * ptree)
{
    Pair look;
    look = SeekItem(pi, ptree);
    if(look.child != NULL)
    {
        printf("name: %s\t\tkind: ", look.child->item.petname);
        for(int i = 0; i < look.child->item.num; i++)
            printf("%s ", look.child->item.kind[i]);
        printf("\n");
    }
    else
        printf("Can not find the pet %s.\n", pi->petname);
}
