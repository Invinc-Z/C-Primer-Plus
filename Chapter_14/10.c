...
void set_confirm(Order_t orders[], int n);
char* s_gets(char* str, int n);

typedef void (*Func)(Order_t orders[], int n); 
Func func[5] = {show_list_empty_seats, show_list_alph_seats, assign_seat, del_seat_assign, set_confirm};
    ...
    while((flight = get_flight() ) != 'q')
    {
        ...
        while((ch = get_choice()) != 'g')
        {
            ...
            case 'a':
                ...
                break;
            case 'b':
                func[0](orders[idx], SEAT_NUM);
                break;
            case 'c':
                func[1](orders[idx], SEAT_NUM);
                break;
            case 'd':
                func[2](orders[idx], SEAT_NUM);
                break;
            case 'e':
                func[3](orders[idx], SEAT_NUM);
                break;
            case 'f':
                func[4](orders[idx], SEAT_NUM);
                break;
            default: 
